...
function Manage({ offers, setOffers, refreshOffers }: ManageProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!draft.institution.trim() || !draft.address.trim() || !draft.description.trim() || !careersList.length) {
  ...
  return <div className="space-y-6">
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-[#003366] via-[#1d3b8c] to-[#2967CD] p-6 text-white shadow-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-[#003366] transition hover:scale-[1.01]">
    ...
    <div className="w-full rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {showForm && <form onSubmit={submit} className="mt-6 w-full space-y-6 rounded-[30px] border border-sky-200 bg-gradient-to-br from-sky-50 via-white to-blue-50 p-8 shadow-inner lg:p-10">
        <div className="flex items-start gap-3 rounded-[22px] border border-sky-100 bg-white/90 p-5">
          ...
          <div>
            ...
            <p className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#003366]">Campos obligatorios: nombre de la empresa, tipo, dirección, mapa, logo, descripción y al menos una carrera.</p>
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">{filteredOffers.length ? filteredOffers.map((offer) => <motion.article initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} key={offer.id} className="rounded-[22px] border border-slate-200 bg-slate-50 p-5">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          <div>
            <h4 className="text-lg font-semibold text-[#003366]">{offer.institution}</h4>
          ...
          <div className="flex gap-2">
            <button onClick={() => edit(offer)} className="inline-flex items-center gap-2 rounded-full bg-[#003366] px-4 py-2 text-sm font-semibold text-white"><PencilLine size={16} /> Editar</button>
...
function Field({ label, value, change, type = 'text', required = false, placeholder, description, textarea = false, max }: { label: string; value: string; change: (value: string) => void; type?: string; required?: boolean; placeholder?: string; description?: string; textarea?: boolean; max?: string }) {
  return <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">{label}</span>{description && <p className="mb-2 text-xs text-slate-500">{description}</p>}{textarea ? <textarea required={required} value={value} onChange={(event) => change(event.target.value)} placeholder={placeholder} className="min-h-[130px] w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-[#2967CD]" /> : <input required={required} type={type} min={type === 'number' ? '0' : undefined} max={max} value={value} onChange={(event) => change(event.target.value)} placeholder={placeholder} className="w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3.5 text-sm outline-none focus:border-[#2967CD]" />}</label>
}
